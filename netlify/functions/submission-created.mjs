 ...
 * When the "audit-download" form is submitted, emails the PDF
 * audit template to the user via Resend.
 *
...
export async function handler(event) {
  ...
  // Only handle the audit-download form
  if (payload.form_name !== "audit-download") {
    return { statusCode: 200, body: "Not audit form — skipping." };
  }
  ...
  try {
    ...
    return { statusCode: 500, body: "Failed to fetch PDF" };
  }
...
<body style="margin:0; padding:0; background-color:#F5F3EF; font-family:Arial, Helvetica, sans-serif;">
        ...
        <table width="580" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border-radius:8px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.06);">
          ...
          <tr>
            <td style="padding:36px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#F5F3EF; border-radius:8px; padding:20px; margin:20px 0;">
                <tr>
  ...
  try {
    ...
    console.log(`Audit PDF emailed to ${email} — Resend ID: ${result.id}`);
